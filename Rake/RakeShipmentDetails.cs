...
namespace Rake
{
    public partial class RakeShipmentDetails : Form
    {
        string EditId = string.Empty;
        int flag = 0;
        DataTable backup = new DataTable();

        public RakeShipmentDetails()
        {
            InitializeComponent();
            BindDropdowns();
            IncrementShipment();
        ...
        private void tsBtnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlConnection con = new SqlConnection(Connection.InvAdminConn());
                SqlCommand cmd = new SqlCommand("InsertShipmentRake", con);
                cmd.CommandType = CommandType.StoredProcedure;
            ...
            {
                SqlConnection con = new SqlConnection(Connection.InvAdminConn());
                SqlCommand cmd = new SqlCommand("UpdateShipmentRake", con);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        private void tsBtnEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlConnection con = new SqlConnection(Connection.InvAdminConn());
                SqlCommand cmd = new SqlCommand("GetShipMentOnIdRake", con);
            ...
            if (!string.IsNullOrEmpty(TxtUploadfile.Text))
            {
                ...
                for (int row = 0; row < dt.Rows.Count; row++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            con = new SqlConnection(Connection.InvAdminConn());
                            cmd = new SqlCommand("InsertShipmentOnUploadRake", con);
                            cmd.CommandType = CommandType.StoredProcedure;
        ...
        {
            string filename = "D:\\RAke\\ShipmentDetails\\ShipmentDetails_" + DateTime.Today.ToString().Substring(0, 10).Replace('/', '_');
            System.IO.Directory.CreateDirectory("D:\\RAke\\ShipmentDetails");
            System.IO.FileInfo excel = new System.IO.FileInfo(filename + ".xlsx");
            ...
            using (var xls = new ExcelPackage(excel))
            {
                ...
                sheet.Cells["K1"].Value = "Shipment No";
                sheet.Cells["L1"].Value = "From";
                sheet.Cells["M1"].Value = "Rake No";
                sheet.Cells["N1"].Value = "Rake Type";
                ...
                sheet.Cells["A1:N" + finalrows.ToString()].Style.WrapText = true;
        ...
        {
            if (!string.IsNullOrEmpty(TxtShipmentSearch.Text))
            {
                GvShipment.DataSource = backup.Select("ShipmentNo Like '%" + TxtShipmentSearch.Text + "%'").Any() ? backup.Select("ShipmentNo Like '%" + TxtShipmentSearch.Text + "%'").CopyToDataTable() : backup.Clone();
            }
            else
            {
