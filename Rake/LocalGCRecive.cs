...
namespace Rake
{
    public partial class LocalGCRecive : Form
    {
        int flag = 0;
        public LocalGCRecive()
        {
            InitializeComponent();
            BindDropdowns();
        }
        ...
        private void tsBtnSave_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(TxtShipment.Text) && string.IsNullOrEmpty(TxtInvoiceNo.Text) && string.IsNullOrEmpty(TxtGCNo.Text))
            {
                MessageBox.Show("Please enter Shipment No or Invoice No or GC No");
                TxtShipment.Focus();
                return;
            }
            else if (string.IsNullOrEmpty(TxtGrosswt.Text))
            {
                MessageBox.Show("Please enter Grosswt");
                TxtGrosswt.Focus();
                return;
            }
            else if (!decimal.TryParse(TxtGrosswt.Text, out b))
            {
                MessageBox.Show("Please enter Correct Grosswt");
                TxtGrosswt.Focus();
                return;
            }
            else if (datetimeShipment.Value>DtRecive.Value)
            {
                MessageBox.Show("Please enter Correct Recive Date");
                DtRecive.Focus();
                return;
            }
            //else if (string.IsNullOrEmpty(TxtShipmentNoRc.Text))
            //{
            //    MessageBox.Show("Please Shipment No");
            //    TxtShipmentNoRc.Focus();
            //    return;
            //}
            else if (string.IsNullOrEmpty(TxtInvoiceNoRc.Text))
            {
                MessageBox.Show("Please Invoice No");
                TxtInvoiceNoRc.Focus();
                return;
            }
            ...
            {
                MessageBox.Show("Please Gross enter wt No");
                TxtGrosswt.Focus();
                return;
            }
            ...
            else if (!DateTime.TryParse(datetimeShipment.Value.ToString(),null,System.Globalization.DateTimeStyles.RoundtripKind,out dt))
            {
                MessageBox.Show("Please select correct Shipment Date");
                datetimeShipment.Focus();
                return;
            }
            ...
            else if (DdlUnLoad.SelectedIndex == 0)
            {
            ...
            SqlConnection con = new SqlConnection(Connection.InvAdminConn());
                SqlCommand cmd = new SqlCommand("UpdateGcRecivedLocal", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@ShipMentID", LblShipmentID.Text);
                cmd.Parameters.AddWithValue("@Material", TxtMaterial.Text);
                cmd.Parameters.AddWithValue("@ShipmentDate", datetimeShipment.Value.Date);
            ...
            cmd.Parameters.AddWithValue("@ShipmentNo", TxtShipmentNoRc.Text);
            cmd.Parameters.AddWithValue("@InvoiceNo", TxtInvoiceNoRc.Text);
        ...
        {
            SqlConnection con = new SqlConnection(Connection.InvAdminConn());
            SqlCommand cmd = new SqlCommand("GetShipMentDetailsForGCLocal", con);
            cmd.Parameters.AddWithValue("@ShipmentNo", TxtShipment.Text);
            ...
            if (dataTable.Rows.Count > 0)
            {
                ...
                LblShipmentID.Text = Convert.ToString(dataTable.Rows[0]["LocalShipmentID"]);
                TxtMaterial.Text = Convert.ToString(dataTable.Rows[0]["Material"]);
                TxtCustomer.Text = Convert.ToString(dataTable.Rows[0]["PartyName"]);
                TxtFrom.Text = Convert.ToString(dataTable.Rows[0]["From"]);
        ...
        {
            string filename = "D:\\Local\\GcRecive\\GcRecive_" + DateTime.Today.ToString().Substring(0, 10).Replace('/', '_');
            System.IO.Directory.CreateDirectory("D:\\Local\\GcRecive");
            ...
            if (!string.IsNullOrEmpty(TxtUploadfile.Text))
            {
                ...
                for (int row = 0; row < dt.Rows.Count; row++)
                {
                    ...
                    try
                    {
                        ...
                        con = new SqlConnection(Connection.InvAdminConn());
                        cmd = new SqlCommand("GetInvoiceOnInvoiceNoLocal", con);
                        ...
                        {
                            con = new SqlConnection(Connection.InvAdminConn());
                            cmd = new SqlCommand("UpdateGcRecivedLocalOnUpload", con);
                            cmd.CommandType = CommandType.StoredProcedure;
