...
namespace Rake
{
    public partial class LocalShipmentDetails : Form
    {
        string EditId = string.Empty;
        int flag = 0;
        DataTable backup = new DataTable();

        public LocalShipmentDetails()
        {
            InitializeComponent();
            BindDropdowns();
            IncrementShipment();
        ...
        public void IncrementShipment()
        {
            ...
            SqlCommand cmd = new SqlCommand("GetMaxShipmentIDLocal", con);
        ...
        {
            SqlConnection con = new SqlConnection(Connection.InvAdminConn());
            SqlCommand cmd = new SqlCommand("GetShipmentLocal", con);
            cmd.CommandType = CommandType.StoredProcedure;
            con.Open();
        ...
        private void tsBtnSave_Click(object sender, EventArgs e)
        {
            ...
            //}
            //else.
            if (!decimal.TryParse(TxtGrossWt.Text, out b))
            {
            ...
            {
                SqlConnection con = new SqlConnection(Connection.InvAdminConn());
                SqlCommand cmd = new SqlCommand("InsertShipmentLocal", con);
                cmd.CommandType = CommandType.StoredProcedure;
            ...
            {
                SqlConnection con = new SqlConnection(Connection.InvAdminConn());
                SqlCommand cmd = new SqlCommand("UpdateShipmentLocal", con);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        private void tsBtnEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlConnection con = new SqlConnection(Connection.InvAdminConn());
                SqlCommand cmd = new SqlCommand("GetShipMentOnIdLocal", con);
                ...
                {
                    TxtShipmentID.Text = Convert.ToString(dataTable.Rows[0]["LocalShipmentID"]);
            ...
            if (!string.IsNullOrEmpty(TxtUploadfile.Text))
            {
                ...
                for (int row = 0; row < dt.Rows.Count; row++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            con = new SqlConnection(Connection.InvAdminConn());
                            cmd = new SqlCommand("InsertShipmentOnUploadLocal", con);
                            cmd.CommandType = CommandType.StoredProcedure;
        ...
        {
            string filename = "D:\\Local\\ShipmentDetails\\ShipmentDetails_" + DateTime.Today.ToString().Substring(0, 10).Replace('/', '_');
            System.IO.Directory.CreateDirectory("D:\\Local\\ShipmentDetails");
            System.IO.FileInfo excel = new System.IO.FileInfo(filename + ".xlsx");
            ...
            using (var xls = new ExcelPackage(excel))
            {
                ...
                //sheet.Cells["A1"].Value = "Shipment No";
                sheet.Cells["A1"].Value = "Invoice No";
                sheet.Cells["B1"].Value = "Shipment Date";
                //sheet.Cells["D1"].Value = "Material";
                sheet.Cells["C1"].Value = "Gross Wt";
                sheet.Cells["D1"].Value = "Product";
                sheet.Cells["E1"].Value = "Customer";
                sheet.Cells["F1"].Value = "From";
                //sheet.Cells["I1"].Value = "Thickness";
                //sheet.Cells["J1"].Value = "Width";
        ...
        private void TxtInvoiceSearch_TextChanged_1(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(TxtInvoiceSearch.Text))
            {
                GvShipment.DataSource = backup.Select("InvoiceNo Like '%" + TxtInvoiceSearch.Text + "%'").Any() ? backup.Select("InvoiceNo Like '%" + TxtInvoiceSearch.Text + "%'").CopyToDataTable() : backup.Clone();
